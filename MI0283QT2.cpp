...
// Add-ons by X. HINAULT - Copyright March 2010
// www.mon-club-elec.fr
// Under GNU GPL License 

//  The next program lines are free software: you can redistribute it and/or modify
...
// one drawInteger method for each data type 
void MI0283QT2::drawInteger(uint16_t x0, uint16_t y0, char n, uint8_t base, uint8_t size,  uint16_t color, uint16_t bg_color)
{
    drawInteger(x0, y0, (long) n, base, size,  color, bg_color);
}

void MI0283QT2::drawInteger(uint16_t x0, uint16_t y0, unsigned char n, uint8_t base, uint8_t size,  uint16_t color, uint16_t bg_color)
{
    drawInteger(x0, y0, (unsigned long) n, base, size,  color, bg_color);
}

void MI0283QT2::drawInteger(uint16_t x0, uint16_t y0, int n, uint8_t base, uint8_t size,  uint16_t color, uint16_t bg_color)
{
   drawInteger(x0, y0, long(n), base, size,  color, bg_color);
}

void MI0283QT2::drawInteger(uint16_t x0, uint16_t y0, unsigned int n, uint8_t base, uint8_t size,  uint16_t color, uint16_t bg_color)
{
   drawInteger(x0, y0, long (n), base, size,  color, bg_color);
}

//--- drawInteger for long - essential method --- 
void MI0283QT2::drawInteger(uint16_t x0, uint16_t y0, long n, uint8_t base, uint8_t size,  uint16_t color, uint16_t bg_color)

{
  if (base == 10) {
    if (n < 0) {
      drawChar( x0, y0,  '-',  size,  color, bg_color);
      x0=x0+(8*size);
      n = -n;
    }
    drawNumber(x0,y0,n,DEC,size,color,bg_color);
    
  } else {
    drawNumber(x0,y0,n,base,size,color,bg_color);
  }
}

void MI0283QT2::drawInteger(uint16_t x0, uint16_t y0, unsigned long n, uint8_t base, uint8_t size,  uint16_t color, uint16_t bg_color)
{
   drawNumber(x0,y0,n,base,size,color,bg_color);
}

//---- suite original method
...
void MI0283QT2::drawFloat(uint16_t x0, uint16_t y0, double number, uint8_t digits, uint8_t size,  uint16_t color, uint16_t bg_color)
{
  ...
  {
     drawChar( x0, y0,  '-',  size,  color, bg_color);
     x0=x0+(8*size); // increment horizontal position
     number = -number;
  }
  ...
  drawInteger(x0, y0,int_part,DEC,size,color,bg_color);

  // Calculate New position x0
  do {
  ...
  // Print the decimal point, but only if there are digits specified beyond
  if (digits > 0){
     drawChar( x0, y0,  '.',  size,  color, bg_color);
      x0=x0+(8*size); // increment horizontal position
  }
  ...
  //  drawInteger(x0, y0,toPrint,DEC,size,color,bg_color);
  //  x0=x0+(8*size); // increment horizontal position
  //  remainder -= toPrint; 
  //} 
  
  // Extract digits from the remainder - another (best?) version
  while (digits-- > 0) // decrement digit on by one
  {
      remainder=remainder*10.0; 
      if (int(remainder)==0) drawChar( x0, y0, '0',  size,  color, bg_color), x0=x0+(8*size); // e.g. 0.000123 with digits=6 give remainder=000123
  }
  
  long toPrint = long(remainder); // for more than 5 decimal places
  if (toPrint>0) drawInteger(x0, y0,toPrint,DEC,size,color,bg_color);
      
} 

//////////// Private Methods - Added by XH --

void MI0283QT2::drawNumber(uint16_t x0, uint16_t y0, unsigned long n, uint8_t base, uint8_t size,  uint16_t color, uint16_t bg_color)
{
  unsigned char buf[8 * sizeof(long)]; // Assumes 8-bit chars. 
  unsigned long i = 0;

  if (n == 0) {
    drawChar( x0, y0,  '0',  size,  color, bg_color); 
    return;
  } 
  ...
  for (; i > 0; i--)
    drawChar( x0, y0,  (char) (buf[i - 1] < 10 ?
    '0' + buf[i - 1] :
     'A' + buf[i - 1] - 10),  size,  color, bg_color), x0=x0+(8*size); 
